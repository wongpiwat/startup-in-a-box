          ...
          {selected.map(item => (
            <div key={item.id} className="flex items-center gap-2 px-4 py-2 rounded-xl bg-card border border-primary/30">
              ...
              {item.confidence_score != null && (
                <span className="text-xs text-primary flex items-center gap-0.5"><Star className="w-3 h-3" />{item.confidence_score}</span>
              )}
          ...
          {selected.length < MAX_COMPARE && (
            <div className="relative">
              <Button
                ...
                className="gap-1.5 border-dashed border-border/80"
              >
                    ...
                    available.map(item => (
                      ...
                      >
                        <div className="flex items-center justify-between">
                          ...
                          {item.confidence_score != null && (
                            <span className="text-xs text-muted-foreground ml-2">{item.confidence_score}/100</span>
                          )}
                        </div>
        ...
        {startups.length >= 2 ? (
          ...
          <div className="text-center py-16">
            ...
            <p className="text-muted-foreground">
              {selected.length === 0
                ? "Add at least 2 startups to start comparing."
                : "Add one more startup to see the comparison."}
            </p>
          </div>
        )}
      </div>
